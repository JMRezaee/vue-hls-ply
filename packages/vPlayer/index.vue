<template>
  <div class="player vplayer">
    <video ref="media" controls></video>
  </div>
</template>
<script>
  ...
  methods: {
    initMedia() {
      ...
      this.media.addEventListener("playing", (e) => {
        this.onMediaPlay();
        this.$emit("onplaying", e);
      });
      this.media.addEventListener("pause", (e) => {
        this.onMediaPause();
        this.$emit("onpause", e);
      });
      this.media.addEventListener("abort", (e) => {
        this.onMediaPause();
        this.$emit("onabort", e);
      });
    ...
    onMediaProgress() {
      if (this.media.buffered.length) {
        this.loadedLength = this.media.buffered.end(
          this.media.buffered.length - 1
        );
      } else {
        this.loadedLength = 0;
      }
      this.$refs.progress.initProgress(
        this.loadedLength,
        this.playedLength,
        this.dataLength
      );
